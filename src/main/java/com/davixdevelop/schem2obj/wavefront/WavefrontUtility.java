...
import com.davixdevelop.schem2obj.wavefront.custom.IAdjacentCheck;
...
public class WavefrontUtility {
     ...
     */
    public static HashMap<String, HashMap<String, String>> texturesToMaterials(List<VariantModels> models, Namespace blockNamespace) {
    ...
    public static ArrayList<Double[]> setAndRotateUVFace(CubeElement.CubeFace face, String orientation, Double[] from, Double[] to){
        ...
        if(face.getRotation() != null){
            ...
            if(face.getUv() == null)
                 rotateUV(UVFace, (orientation.equals("south") ||  orientation.equals("north")) ? -face.getRotation() : face.getRotation(), faceOrigin);
     ...
     * @return
     */
    public static void rotateUV(ArrayList<Double[]> uvs, Double angle, Double[] rotationOrigin){
     ...
     */
    public static String[] getCornerPerOrientation(String orientation){
    ...
    public static ArrayList<Integer[]> createWavefrontFace(Map<String, Integer> VerticesIndexes, Integer[] UVIndexes, String orientation) {
        String[] corners = getCornerPerOrientation(orientation);
    ...
    public static Map<String, Double[]> createCubeVerticesFromPoints(Double[] A, Double[] F, Set<String> faces, ArrayVector.MatrixRotation rotationX, ArrayVector.MatrixRotation rotationY){
        ...
        {
            //Check height
            increaseOverlapingCube(A, F, 2);
            //Check length
            increaseOverlapingCube(A, F, 1);
            //Check width
            increaseOverlapingCube(A, F, 0);
        }
     ...
     * @param axisIndex
     */
    private static void increaseOverlapingCube(Double[] A, Double[] F, int axisIndex){
     ...
     * @param wavefrontBlock The block to translate
     ...
     */
    public static int getFacePlane(Double plane[], ArrayList<Double[]> vertices){
        Boolean x = null;
        Boolean y = null;
        Boolean z = null;
    ...
    public static void avoidOverlapping(String orientation, Map<String, Double[]> cubeCorners, HashedDoubleList vertices, HashMap<String, ArrayList<ArrayList<Integer[]>>> faces){
        ...
        //Loop thorough faces and check if they overlap
        for(String mat : faces.keySet()){
            ArrayList<ArrayList<Integer[]>> matFaces = faces.get(mat);

            for(ArrayList<Integer[]> face : matFaces){
                ArrayList<Double[]> faceVertices2 = new ArrayList<>();
                for(Integer[] indices : face)
                    faceVertices2.add(vertices.get(indices[0]));
    ...
    *-------------           -------A-----
    ...
    public static void convertCubeToWavefront(CubeElement element, boolean uvLock, ArrayVector.MatrixRotation rotationX, ArrayVector.MatrixRotation rotationY,  HashedDoubleList vertices, HashedDoubleList textureCoordinates, HashMap<String, ArrayList<ArrayList<Integer[]>>> faces, HashMap<String, HashMap<String, ArrayList<Integer>>> boundingFaces, HashMap<String, String> modelsMaterials){
        ...
        Map<String, Double[]> cubeCorners = WavefrontUtility.createCubeVerticesFromPoints(from, to, element.getFaces().keySet(), rotationX, rotationY);
        ...
        for (String key : elementFaces.keySet()) {
            ...
            if (uvLock) {
                if (rotationX != null) {
                    ...
                    if (key.equals("west") || key.equals("east"))
                        WavefrontUtility.rotateUV(faceUV, rotationX.getRot(), new Double[]{0.5, 0.5, 0.0});
                    else {
                ...
                if (rotationY != null) {
                    ...
                    if (orientationCoord.get(2) == 1 || orientationCoord.get(2) == -1)
                         WavefrontUtility.rotateUV(faceUV, rotationY.getRot(), new Double[]{0.5, 0.5, 0.0});
                    else {
                        /*//Else rotate orientationCoord by closest right angle (ex, 100 -> 90)
            ...
            String faceMaterial = materialPerOrientation.get((!uvLock || materialPerOrientation.size() == 1) ? WavefrontUtility.orientationToCoords(key) : ((rotationX != null && (orientationCoord.get(2) != 1 || orientationCoord.get(2) != -1)) || (rotationY != null && ( orientationCoord.get(2) == 1 || orientationCoord.get(2) == -1)) ? orientationCoord : WavefrontUtility.orientationToCoords(key)));
